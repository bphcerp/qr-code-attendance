...
async function main() {
  ...
  // One mark per student per session is the replay defence: a token is valid for
  // every enrolled student at once, so reuse is prevented per-student rather than
  // per-token, enforced by the unique index the mark route turns into 409.
  ...
  check('an enrolled student can mark', first.status === 200)

  const second = await mark(student2Cookie)
  check('a second enrolled student can mark in the same session', second.status === 200)

  const repeat = await mark(studentCookie)
  check('the same student scanning twice is already_marked', repeat.status === 409)

  const again = await mark(student2Cookie)
  check('a student who already marked cannot mark again', again.status !== 200)
